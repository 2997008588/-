...
# ---------------------------
# Robust chunk accessors (Chunk / dict / tuple / arbitrary object)
# ---------------------------
def _cget(c: Any, key: str, default=None):
    if c is None:
        return default
...
def c_chunk_id(c: Any, fallback: str = "") -> str:
    return str(_cget(c, "chunk_id", fallback) or fallback)


def c_file_name(c: Any, fallback: str = "unknown.pdf") -> str:
    return str(_cget(c, "file_name", _cget(c, "source", fallback)) or fallback)


def c_page_start(c: Any, fallback: int = 1) -> int:
...
def c_page_end(c: Any, fallback: int = None) -> int:
    if fallback is None:
...
def normalize_chunks(raw: Any) -> List[Chunk]:
    """
    兼容旧版 meta.pkl：
    - list[Chunk]
    ...
    if isinstance(raw, dict):
        ...
        # 若 key 都像数字，按数字排序取 values（尽量保持稳定）
    ...
    return [as_chunk(it, idx=i) for i, it in enumerate(items)]
...
def infer_corpus_topic(chunks: List[Chunk], sample_n: int = 80) -> str:
    ...
    sample_text = "\n".join(c_text(chunks[i]) for i in range(0, len(chunks), step))[:200000]
...
def save_index(index: faiss.Index, chunks: List[Chunk]):
    """
    关键：不要直接 pickle Chunk 类对象（Streamlit Cloud 可能触发 PicklingError），
    统一保存为 list[dict]（基本类型 100% 可序列化）。
    """
    ...
    for i, c in enumerate(chunks):
        meta.append({
            "chunk_id": c.chunk_id if getattr(c, "chunk_id", None) else f"unknown::c{i:06d}",
            "file_name": c.file_name if getattr(c, "file_name", None) else "unknown.pdf",
            "page_start": int(getattr(c, "page_start", 1) or 1),
            "page_end": int(getattr(c, "page_end", getattr(c, "page_start", 1) or 1) or 1),
...
def load_index() -> Tuple[faiss.Index, List[Chunk]]:
    ...
    # 始终写回为 list[dict]，避免后续再出现 pickling 问题
    try:
        meta = [{
            "chunk_id": c.chunk_id,
        ...
        with open(META_PATH, "wb") as wf:
            pickle.dump(meta, wf, protocol=pickle.HIGHEST_PROTOCOL)
    except Exception:
        pass
    ...
    if "index" not in st.session_state or "chunks" not in st.session_state:
        with st.spinner("加载索引..."):
            try:
                ...
                st.warning(f"索引加载失败，尝试自动重建：{repr(e)}")
                ok = do_build()
                if not ok:
                    st.stop()
                st.session_state.index, st.session_state.chunks = load_index()

        # 对齐校验：索引向量数应与 meta 数一致，否则重建
        try:
            nt = int(getattr(st.session_state.index, "ntotal", 0))
            if nt and len(st.session_state.chunks) and nt != len(st.session_state.chunks):
                st.warning(f"检测到索引与元数据长度不一致（index={nt}, meta={len(st.session_state.chunks)}），正在自动重建索引…")
                ok = do_build()
                if not ok:
                    st.stop()
                st.session_state.index, st.session_state.chunks = load_index()
